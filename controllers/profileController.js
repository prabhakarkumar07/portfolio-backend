...
const updateProfile = async (req, res, next) => {
  try {
    ...
    if (availabilityBadge !== undefined)
      profile.availabilityBadge = availabilityBadge;
    if (availabilityStatus !== undefined)
      profile.availabilityStatus = availabilityStatus;
    if (availabilityDetails !== undefined)
      profile.availabilityDetails = availabilityDetails;
    if (heroDescription !== undefined)
      profile.heroDescription = heroDescription;
    if (homeCtaTitle !== undefined) profile.homeCtaTitle = homeCtaTitle;
    if (homeCtaDescription !== undefined)
      profile.homeCtaDescription = homeCtaDescription;
    if (homePrimaryCtaText !== undefined)
      profile.homePrimaryCtaText = homePrimaryCtaText;
    if (homeSecondaryCtaText !== undefined)
      profile.homeSecondaryCtaText = homeSecondaryCtaText;
    ...
    if (projectsPageTag !== undefined)
      profile.projectsPageTag = projectsPageTag;
    if (projectsPageTitle !== undefined)
      profile.projectsPageTitle = projectsPageTitle;
    if (projectsPageSubtitle !== undefined)
      profile.projectsPageSubtitle = projectsPageSubtitle;
    ...
    if (careerJourney !== undefined) {
      profile.careerJourney = Array.isArray(careerJourney)
        ? careerJourney
            .filter(
              (item) => item && (item.year || item.title || item.description),
            )
            .map((item) => ({
              year: item.year || "",
              title: item.title || "",
  ...
  try {
    ...
    if (profileImage) {
      ...
      profile.profileImage = {
        ...
        accessMode: uploadedImage.access_mode || "public",
        deliveryType: uploadedImage.type || "upload",
      };
    }
